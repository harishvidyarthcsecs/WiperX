...
| `test_carver_fragment.py` | 6 | Contiguous byte-exact recovery, bifragment recovery at 3 gap sizes, random-sized gap, garbage-region rejection, truncated-input (no EOI) rejection |
...
| `test_web.py` | 9 | Anonymous redirect, Admin page access, dashboard action cards, audit-log view, Viewer RBAC denial (recovery page + run + audit log), malformed recovery-run request, unknown-case 404 |
